...
public class ReviewController {
    ...
    public ResponseEntity<ApiResponse<MentorReviewResponse>> getMentorReviews(@PathVariable Long mentorId,
        ...
        MentorReviewResponse response = reviewService.findMentorReviews(mentorId, pageable);
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponse.<MentorReviewResponse>builder().success(200).message("멘토 리뷰 조회에 성공했습니다.").data(response).build());
    }
}
